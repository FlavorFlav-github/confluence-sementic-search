...
- rag-qdrant: Vector database for storing embeddings (port 6333).

- rag-cache: Redis cache for query results and embeddings (port 6379).

- rag-indexer: Background service that indexes Confluence pages based on rag_config.yaml.

- rag-app: Main API service for querying the indexed documentation and ask the RAG (port 8000).
...
Indexing Parameters: Configure chunk size, overlap, and batch processing settings for optimal embedding generation.​

Embedding Models: Specify which embedding model to use for vector generation.​

Cache Settings: Configure Redis cache behavior including TTL and enabled features.​
...
Embedding Cache: Stores computed embeddings to avoid recomputation for similar queries.​

Query Cache: Caches search results for frequently asked questions, reducing vector database queries.​

Semantic Cache: Uses vector similarity to match queries with similar meanings, serving pre-approved answers instantly.​

Response Cache: Stores LLM-generated responses to reduce API calls and costs by up to 80%.​

Cache hit rates and performance metrics can be monitored through application logs.​

🐳 Docker Services
The Docker Compose setup includes four services:​

rag-app: Main API service exposing the RAG query endpoint on port 8000.

rag-indexer: Background service that continuously indexes Confluence documentation based on rag_config.yaml.​

rag-qdrant: Qdrant vector database for storing and querying embeddings (port 6333).​

rag-cache: Redis cache for query results, embeddings, and LLM responses (port 6379).​
...
Developed by Yassine/@FlavorFlav-github